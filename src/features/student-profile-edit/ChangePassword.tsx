...
function ChangePassword() {
    ...
    const cannotSubmit =
        oldPasswordInput.hasErrors || newPasswordInput.hasErrors;

    const save = () => {
        oldPasswordInput.blur();
        newPasswordInput.blur();
        if (
            [oldPasswordInput, newPasswordInput].some((input) => !input.isValid)
        ) {
            return;
        }
    ...
    return (
        <>
            ...
            <p className="text-sm text-darkGray mb-2">
                Зміна паролю не вплине на інші пристрої, з яких ви входили в
                систему
            </p>
            <hr />
