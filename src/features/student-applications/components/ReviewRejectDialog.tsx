...
import { useRejectStudentApplicationMutation } from "@/features/student-applications/hooks/use-application-review-query";
...
export default function ReviewRejectDialog({
  ...
  const rejectMutation = useRejectStudentApplicationMutation();
  ...
  const handleSubmit = async () => {
    ...
    await rejectMutation.mutateAsync({
      reviewId,
      message: noFeedback.value
        ? "Для цього подання не було залишено повідомлення"
        : messageInput.value,
    });
    onClose();
  };
