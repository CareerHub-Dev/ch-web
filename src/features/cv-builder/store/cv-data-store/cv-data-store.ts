...
export type CvDataStore = {
    ...
    dispatchForeignLanguages: (
        action: ArrayInputAction<ForeignLanguage>
    ) => void;
    ...
    devtools(
        immer((set) => ({
            ...
            discardChanges: (lastSave) =>
                set((state) => {
                    ...
                    if (typeof lastSave.title === "string") {
                        state.cvData = restoreToCvQueryData(
                            lastSave as CvQueryData
                        );
                    }
            ...
            dispatchTitle: (action) =>
                set((state) => {
                    state.cvData.title = titleReducer(
                        state.cvData.title,
                        action
                    );
                }),
            ...
            dispatchGoals: (action) =>
                set((state) => {
                    state.cvData.goals = goalsReducer(
                        state.cvData.goals,
                        action
                    );
                }),
